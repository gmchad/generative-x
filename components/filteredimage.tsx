...
export function FilteredImage(props: {
  ...
  React.useEffect(() => {
    ...
    async function getFilteredImage(
      ...
      try {
        ...
        if (data.url && data.url?.startsWith("http")) {
          ...
          if (!ac.signal.aborted) setFilteredImageUrl(data.url);
        } else if (data.image) {
          ...
          // Replace the image in the UI only if the client hasn't moved on since
          if (!ac.signal.aborted) setFilteredImageUrl(base64ImageUrl);
        } else {
    ...
    const timeoutId = setTimeout(
      () => getFilteredImage(imageUrl, styleText, _ac),
      0,
    );
    return () => {
      _ac.abort();
  ...
  if (!styleText)
    return (
      <picture>
        <img src={imageUrl} alt={props.altText} className={props.className} />
      </picture>
    );
      ...
      {loading ? (
        ...
        <picture>
          <img
            src={imageUrl}
            alt={props.altText}
            className={props.className + " blur-lg grayscale"}
          />
        </picture>
      ) : (
